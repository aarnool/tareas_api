...
def create_task(
    ...
    """Crea una nueva tarea en base de datos vinculada al usuario autenticado."""
...
def get_all_tasks(
    ...
    """Obtiene el listado paginado de tareas correspondientes al usuario actual."""
    user_id = current_user["user_id"]
...
Updates fields of an existing task by its ID. Only the task owner can update it.
Actualiza los campos de una tarea existente por su ID. Solo el propietario puede modificarla.
""",
    response_model=TaskResponse,
    status_code=status.HTTP_200_OK,
...
def delete_task(
    ...
    """Elimina la tarea especificada por ID si pertenece al usuario actual."""
    user_id = current_user["user_id"]
