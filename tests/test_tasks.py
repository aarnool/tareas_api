...
def test_tasks_user_isolation(client):
    ...
    # User 2 gets tasks -> should only see Tarea de Usuario 2
    ...
    # User 2 tries to update User 1's task -> should be 404
    patch_res = client.patch(f"/tasks/{task_id_u1}", json={"title": "Hacked"})
    assert patch_res.status_code == 404

    # User 2 tries to delete User 1's task -> should be 404
